...
class User extends BaseModel {
    ...
    public function updateProfile($id,$name,$phone,$bio,$pic=null) {
        ...
        if ($pic) {
            ...
            return $this->execute(
                'UPDATE users SET name=?, phone=?, bio=?, profile_pic=? WHERE id=?',
                'ssssi',
                [$name,$phone,$bio,$pic,$id]
            );
        }
    ...
    public function all($keyword='') {
        ...
        return $this->rows(
            'SELECT * FROM users WHERE name LIKE ? OR email LIKE ? OR role LIKE ? ORDER BY created_at DESC',
            'sss',
            [$like,$like,$like]
        );
    }
    ...
    public function approveSeller($id) {
        ...
        return $this->execute(
            'UPDATE users SET role=\'seller\', seller_verified=1 WHERE id=?',
            'i',
            [$id]
        );
    }
    ...
    public function revokeSeller($id) {
        ...
        return $this->execute(
            'UPDATE users SET seller_verified=0, role=\'buyer\' WHERE id=?',
            'i',
            [$id]
        );
    }
